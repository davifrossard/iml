...
with tf.name_scope("Hidden_Layer"):
    HW = tf.Variable(init_param([784, 300]))
    HB = tf.Variable(init_param([300]))
    HZ = tf.matmul(inputs, HW) + HB
    HA = tf.nn.tanh(HZ)
    
with tf.name_scope("Output_Layer"):
    OW = tf.Variable(init_param([300, 10]))
    OB = tf.Variable(init_param([10]))
    logits = tf.matmul(HA, OW) + OB
